...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'Preproccess'))
...
def train_bone_age_model(model, optimizer, train_loader, val_loader,test_loader, device,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch == 5:
            ...
            for p in model.contrastive_model.backbone[7].parameters():
            ...
            optimizer = torch.optim.AdamW(
                [
                    ...
                    {"params": model.pre_head_norm.parameters(), "lr": 1e-3},
                    {"params": model.gender_embed.parameters(), "lr": 1e-3},
                    # 🔥 NEW HEAD PARAMS
        ...
        for it, batch in enumerate(train_loader):
            ...
            pred_age = bin_center + pred_offset.squeeze(1)
...
if __name__ == '__main__':
    ...
    print("[Sanity] reg in_features = 512 + gender_embed(8) =", 512 + 8)
